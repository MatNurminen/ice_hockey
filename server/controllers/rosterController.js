...
const sqlRoster = {
  text: 'SELECT championship.championship_id, player.pos, player.num, player.first_name, player.last_name, country.flag, player.birth, \
      ...
      player.player_id AS pl_id, ($1 - player.birth) AS age, draft.logo as draftlogo \
      FROM championship \
      ...
      LEFT JOIN club_logo draft ON (draft.club_id = player.draft) \
      ...
      AND (draft.end_year >= 2020 OR draft.end_year IS NULL) \
...
const get_roster = async (req, res) => {
...
module.exports = { get_roster, delete_from_roster, insert_into_roster };
